...
class DuelScheduler {
  ...
  async getCaptions(image1Id, image2Id) {
    try {
      ...
      for (const row of result.rows) {
        if (row.image_id === image1Id) {
          captions.image1.push(row.caption);
        } else if (row.image_id === image2Id) {
          captions.image2.push(row.caption);
        }
      }
  ...
  /**
   * Update the duel message with new captions
   * @param {string} guildId - Guild ID
   */
  async updateDuelMessage(guildId) {
    try {
      ...
      if (!config.duel_channel_id) return;

      const activeDuel = await duelManager.getActiveDuel(guildId);
      if (!activeDuel || !activeDuel.messageId) return;

      // Get channel and message
      const channel = await this.client.channels.fetch(config.duel_channel_id);
      if (!channel) return;

      const message = await channel.messages.fetch(activeDuel.messageId);
      if (!message) return;

      // Get signed URLs
      const image1Url = await storage.getImageUrl(activeDuel.image1.s3_key);
      ...
      const duelData = {
        image1: activeDuel.image1,
        image2: activeDuel.image2,
        isWildcard: activeDuel.isWildcard
      };
      
      const embeds = embedUtils.createDuelEmbed(duelData, image1Url, image2Url, new Date(activeDuel.endsAt), captions);

      // Update the message
      await message.edit({ embeds: embeds });
      
      console.log(`✅ Updated duel message for guild ${guildId} with new captions`);
    } catch (error) {
      console.error(`Error updating duel message for guild ${guildId}:`, error);
    }
  }
  ...
  async startDuel(guildId) {
    try {
      ...
      // Create embeds with empty captions (it's a new duel)
      const embeds = embedUtils.createDuelEmbed(duelData, image1Url, image2Url, duelData.endsAt, { image1: [], image2: [] });
...
export default DuelScheduler;import { ActionRowBuilder, ButtonBuilder, ButtonStyle } from 'discord.js';
import database from '../database/database.js';
import duelManager from './duelManager.js';
import storage from './storage.js';
import embedUtils from '../utils/embeds.js';
