/**
 * Duel Manager - DEBUG VERSION
 * Added logging to see what's being sent to Discord
 */
...
class DuelManager {
  ...
  async postDuelMessage(guildId, config, pair, endsAt) {
    try {
      ...
      console.log('🔍 DEBUG: Creating embeds...');
      const embeds = embedUtils.createDuelEmbed(pair.image1, pair.image2, url1, url2, endsAt);
      
      console.log('🔍 DEBUG: Embeds type:', typeof embeds);
      console.log('🔍 DEBUG: Is array?', Array.isArray(embeds));
      console.log('🔍 DEBUG: Embeds length:', embeds?.length);
      console.log('🔍 DEBUG: First embed:', embeds?.[0]);

      const row = embedUtils.createVoteButtons(pair.image1.id, pair.image2.id);

      console.log('🔍 DEBUG: Sending message with embeds...');
